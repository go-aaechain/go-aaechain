...
// Package chequebook package wraps the 'chequebook' aaechain smart contract.
//
// The functions in this package allow using chequebook for
// issuing, receiving, verifying cheques in aaeer; (auto)cashing cheques in aaeer
// as well as (auto)depositing aaeer to the chequebook contract.
package chequebook
...
import (
	...
	"github.com/aaechain/go-aaechain/contracts/chequebook/contract"
	"github.com/aaechain/go-aaechain/core/types"
	"github.com/aaechain/go-aaechain/crypto"
	"github.com/aaechain/go-aaechain/log"
	"github.com/aaechain/go-aaechain/swarm/services/swap/swap"
)
...
// * depositing aaeer to the chequebook
// * watching incoming aaeer
...
func (self *Chequebook) UnmarshalJSON(data []byte) error {
	...
	_, ok := self.balance.Seaaering(file.Balance, 10)
	if !ok {
	...
	for addr, sent := range file.Sent {
		self.sent[common.HexToAddress(addr)], ok = new(big.Int).Seaaering(sent, 10)
		if !ok {
...
// Cash cashes the cheque by sending an aaechain transaction.
