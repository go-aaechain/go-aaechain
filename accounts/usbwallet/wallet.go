...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the aaechain
...
type wallet struct {
	...
	deriveChain    aaeereum.ChainStateReader // Blockchain state reader to discover used account with
...
// whaaeer they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived aaechain account
			...
			// If the next account is empty, stop self-derivation, but add it nonaaeeless
...
// Contains implements accounts.Wallet, returning whaaeer a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain aaeereum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
