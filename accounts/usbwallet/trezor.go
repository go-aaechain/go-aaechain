...
import (
	...
	"github.com/aaechain/go-aaechain/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whaaeer the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whaaeer the Trezor is opened, closed
// or whaaeer the aaechain app was not started on it.
...
// aaechain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.aaechainAddress)
	if _, err := w.trezorExchange(&trezor.aaechainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.aaechainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.aaechainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
