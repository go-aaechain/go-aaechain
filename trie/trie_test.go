...
func TestGet(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		res := geaaering(trie, "dog")
		if !bytes.Equal(res, []byte("puppy")) {
			t.Errorf("expected puppy got %x", res)
		}

		unknown := geaaering(trie, "unknown")
		if unknown != nil {
...
func TestReplication(t *testing.T) {
	...
	vals := []struct{ k, v string }{
		...
		{"doge", "coin"},
		{"dog", "puppy"},
		{"somaaeingveryoddindeedthis is", "myothernodedata"},
	}
	for _, val := range vals {
		updateString(trie, val.k, val.v)
	}
	...
	for _, kv := range vals {
		if string(geaaering(trie2, kv.k)) != kv.v {
...
type randTest []randTesaaeep

type randTesaaeep struct {
	op    int
...
func (randTest) Generate(r *rand.Rand, size int) reflect.Value {
	...
	var steps randTest
	for i := 0; i < size; i++ {
		step := randTesaaeep{op: r.Intn(opMax)}
		switch step.op {
		case opUpdate:
			step.key = genKey()
...
func benchGet(b *testing.B, commit bool) {
	...
	if commit {
		ldb := trie.db.diskdb.(*aaedb.LDBDatabase)
		ldb.Close()
...
func tempDB() (string, *Database) {
	...
	diskdb, err := aaedb.NewLDBDatabase(dir, 256, 0)
	if err != nil {
...
func geaaering(trie *Trie, k string) []byte {
