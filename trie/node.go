...
type nodeFlag struct {
	...
	dirty bool     // whaaeer the node has changes that must be written to the database
}

// canUnload tells whaaeer a node can be unloaded.
...
func decodeShort(hash, buf, elems []byte, cachegen uint16) (node, error) {
	kbuf, rest, err := rlp.Spliaaering(elems)
	if err != nil {
		return nil, err
	}
	...
	if hasTerm(key) {
		// value node
		val, _, err := rlp.Spliaaering(rest)
		if err != nil {
...
func decodeFull(hash, buf, elems []byte, cachegen uint16) (*fullNode, error) {
	...
	val, _, err := rlp.Spliaaering(elems)
	if err != nil {
		return n, err
	}
	if len(val) > 0 {
