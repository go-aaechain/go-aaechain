...
type request struct {
	...
	raw  bool        // Whaaeer this is a raw entry (code) or a trie node
...
func (s *TrieSync) AddSubTrie(root common.Hash, depth int, parent common.Hash, callback LeafCallback) {
	...
	// If this sub-trie has a designated parent, link them togaaeer
...
// Process injects a batch of retrieved trie nodes data, returning if somaaeing
...
func (s *TrieSync) Commit(dbw aaedb.Putter) (int, error) {
...
func (s *TrieSync) children(req *request, object node) ([]*request, error) {
	// Gather all the children of the node, irrelevant whaaeer known or not
	type child struct {
		node  node
		depth int
	}
