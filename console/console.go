...
import (
	...
	"github.com/aaechain/go-aaechain/internal/jsre"
	"github.com/aaechain/go-aaechain/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute aaechain requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jaae", struct{}{})

	jaaeObj, _ := c.jsre.Get("jaae")
	jaaeObj.Object().Set("send", bridge.Send)
	jaaeObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jaae);"); err != nil {
	...
	flatten := "var aae = web3.aae; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = aae.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jaae.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jaae.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jaae.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jaae.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jaae.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines aae.getBalance(aae.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gaae instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gaae metadata
	fmt.Fprintf(c.printer, "Welcome to the Gaae JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + aae.coinbase);
		console.log("at block: " + aae.blockNumber + " (" + new Date(1000 * aae.getBlock(aae.blockNumber).timestamp) + ")");
