...
func newTester(t *testing.T, confOverride func(*aae.Config)) *tester {
	...
	// Create a networkless protocol stack and start an aaechain service within
	...
	aaeConf := &aae.Config{
		...
		aaeerbase: common.HexToAddress(testAddress),
		aaeash: ethash.Config{
			PowMode: ethash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(aaeConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return aae.New(ctx, aaeConf) }); err != nil {
		t.Fatalf("failed to register aaechain protocol: %v", err)
	}
