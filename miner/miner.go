...
// Package miner implements aaechain block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whaaeer we can start the mining operation
	shouldStart int32 // should start indicates whaaeer we should start after sync
}

func New(aae Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		aae:      aae,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, aae, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(aae.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setaaeerbase(coinbase)
...
func (self *Miner) Setaaeerbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setaaeerbase(addr)
}
