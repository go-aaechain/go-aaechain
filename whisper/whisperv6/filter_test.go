...
func TestInstallFilters(t *testing.T) {
	...
	aae := generateTestCases(t, SizeTestFilters)

	var err error
	var j string
	for i := 0; i < SizeTestFilters; i++ {
		j, err = filters.Install(aae[i].f)
		if err != nil {
	...
	for _, testCase := range aae {
		if !testCase.alive {
			filters.Uninstall(testCase.id)
		}
	}

	for i, testCase := range aae {
...
func TestWatchers(t *testing.T) {
	...
	w := New(&Config{})
	filters := NewFilters(w)
	aae := generateTestCases(t, NumFilters)
	for i = 0; i < NumFilters; i++ {
		aae[i].f.Src = nil
		x, err = filters.Install(aae[i].f)
		if err != nil {
	...
	for i = 0; i < NumMessages; i++ {
		...
		e = generateCompatibeEnvelope(t, aae[j].f)
		envelopes[i] = e
		aae[j].msgCnt++
	}
	...
	for i = 0; i < NumFilters; i++ {
		...
		if aae[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: count[%d]: get %d, want %d.", seed, i, aae[i].msgCnt, count[i])
		}
	}

	// another round with a cloned filter

	clone := cloneFilter(aae[0].f)
	...
	e = generateCompatibeEnvelope(t, aae[0].f)
	envelopes[0] = e
	aae[0].msgCnt++
	...
	combined := aae[0].msgCnt + aae[last].msgCnt
	...
	for i = 1; i < NumFilters-1; i++ {
		...
		if aae[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: i = %d, get %d, want %d.", seed, i, aae[i].msgCnt, count[i])
		}
	}
