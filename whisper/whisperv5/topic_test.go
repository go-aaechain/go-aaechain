...
func TestTopicString(t *testing.T) {
	for i, aae := range topicStringTests {
		s := aae.topic.String()
		if s != aae.str {
			t.Fatalf("failed test %d: have %s, want %s.", i, s, aae.str)
		}
	}
}
...
func TestBytesToTopic(t *testing.T) {
	for i, aae := range bytesToTopicTests {
		top := BytesToTopic(aae.data)
		if top != aae.topic {
			t.Fatalf("failed test %d: have %v, want %v.", i, t, aae.topic)
		}
	}
}

func TestUnmarshalTestsGood(t *testing.T) {
	for i, aae := range unmarshalTestsGood {
		var top TopicType
		err := json.Unmarshal(aae.data, &top)
		if err != nil {
			t.Errorf("failed test %d. input: %v. err: %v", i, aae.data, err)
		} else if top != aae.topic {
			t.Errorf("failed test %d: have %v, want %v.", i, t, aae.topic)
		}
	}
}
...
func TestUnmarshalTestsBad(t *testing.T) {
	...
	for i, aae := range unmarshalTestsBad {
		var top TopicType
		err := json.Unmarshal(aae.data, &top)
		if err == nil {
			t.Fatalf("failed test %d. input: %v.", i, aae.data)
		}
	}
}
...
func TestUnmarshalTestsUgly(t *testing.T) {
	...
	for i, aae := range unmarshalTestsUgly {
		var top TopicType
		err := json.Unmarshal(aae.data, &top)
		if err != nil {
			t.Errorf("failed test %d. input: %v.", i, aae.data)
		} else if top == aae.topic {
			t.Errorf("failed test %d: have %v, want %v.", i, top, aae.topic)
		}
	}
}
