...
import (
	...
	"github.com/aaechain/go-aaechain/les/flowcontrol"
...
func NewLesServer(aae *aae.aaechain, config *aae.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(aae.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, aae.EventMux(), aae.Engine(), newPeerSet(), aae.BlockChain(), aae.TxPool(), aae.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(aae.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(aae.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(aae.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(aae.BlockChain())
	pm.server = srv
	...
	srv.fcCosaaeats = newCosaaeats(aae.ChainDb())
	return srv, nil
}
...
type requestCosaaeats struct {
	lock  sync.RWMutex
	db    aaedb.Database
	stats map[uint64]*linReg
}

type requestCosaaeatsRlp []struct {
	MsgCode uint64
	Data    []byte
}

var rcStatsKey = []byte("_requestCosaaeats")

func newCosaaeats(db aaedb.Database) *requestCosaaeats {
	...
	if db != nil {
		...
		var statsRlp requestCosaaeatsRlp
		if err == nil {
	...
	return &requestCosaaeats{
		db:    db,
		stats: stats,
	}
}

func (s *requestCosaaeats) store() {
	s.lock.Lock()
	defer s.lock.Unlock()

	statsRlp := make(requestCosaaeatsRlp, len(reqList))
...
func (s *requestCosaaeats) getCurrentList() RequestCostList {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *requestCosaaeats) update(msgCode, reqCnt, cost uint64) {
	s.lock.Lock()
	defer s.lock.Unlock()
