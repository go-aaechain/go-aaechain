...
const (
	...
	// iniaaeatsWeight is used to initialize previously unknown peers with good
...
type serverPool struct {
	...
	adjusaaeats          chan poolStatAdjust
...
func newServerPool(db aaedb.Database, quit chan struct{}, wg *sync.WaitGroup) *serverPool {
	pool := &serverPool{
		db:           db,
		quit:         quit,
		wg:           wg,
		...
		adjusaaeats:  make(chan poolStatAdjust, 100),
...
func (pool *serverPool) disconnect(entry *poolEntry) {
	...
	if entry.state == psRegistered {
		...
		if stopped {
			entry.connecaaeats.add(1, connAdjust)
		} else {
			entry.connecaaeats.add(connAdjust, 1)
		}
	}
...
func (pool *serverPool) adjustBlockDelay(entry *poolEntry, time time.Duration) {
	...
	pool.adjusaaeats <- poolStatAdjust{pseBlockDelay, entry, time}
}
...
func (pool *serverPool) adjustResponseTime(entry *poolEntry, time time.Duration, timeout bool) {
	...
	if timeout {
		pool.adjusaaeats <- poolStatAdjust{pseResponseTimeout, entry, time}
	} else {
		pool.adjusaaeats <- poolStatAdjust{pseResponseTime, entry, time}
	}
}
	...
	for {
		...
		case adj := <-pool.adjusaaeats:
			...
			case pseResponseTime:
				...
				adj.entry.timeouaaeats.add(0, 1)
			case pseResponseTimeout:
				adj.entry.timeouaaeats.add(1, 1)
			}
			pool.lock.Unlock()
...
func (pool *serverPool) findOrNewNode(id discover.NodeID, ip net.IP, port uint16) *poolEntry {
	...
	if entry == nil {
		...
		entry.connecaaeats.add(1, iniaaeatsWeight)
		entry.delayStats.add(0, iniaaeatsWeight)
		entry.responseStats.add(0, iniaaeatsWeight)
		entry.timeouaaeats.add(0, iniaaeatsWeight)
	}
...
func (pool *serverPool) loadNodes() {
	...
	for _, e := range list {
		log.Debug("Loaded server stats", "id", e.id, "fails", e.lastConnected.fails,
			"conn", fmt.Sprintf("%v/%v", e.connecaaeats.avg, e.connecaaeats.weight),
			"delay", fmt.Sprintf("%v/%v", time.Duration(e.delayStats.avg), e.delayStats.weight),
			"response", fmt.Sprintf("%v/%v", time.Duration(e.responseStats.avg), e.responseStats.weight),
			"timeout", fmt.Sprintf("%v/%v", e.timeouaaeats.avg, e.timeouaaeats.weight))
		pool.entries[e.id] = e
...
type poolEntry struct {
	...
	connecaaeats, delayStats    poolStats
	responseStats, timeouaaeats poolStats
	state                       int
...
func (e *poolEntry) EncodeRLP(w io.Writer) error {
	return rlp.Encode(w, []interface{}{e.id, e.lastConnected.ip, e.lastConnected.port, e.lastConnected.fails, &e.connecaaeats, &e.delayStats, &e.responseStats, &e.timeouaaeats})
}
...
func (e *poolEntry) DecodeRLP(s *rlp.Stream) error {
	var entry struct {
		...
		Port                       uint16
		Fails                      uint
		CStat, DStat, RStat, aaeat poolStats
	}
	...
	e.connecaaeats = entry.CStat
	e.delayStats = entry.DStat
	e.responseStats = entry.RStat
	e.timeouaaeats = entry.aaeat
...
func (e *knownEntry) Weight() int64 {
	...
	return int64(1000000000 * e.connecaaeats.recentAvg() * math.Exp(-float64(e.lastConnected.fails)*failDropLn-e.responseStats.recentAvg()/float64(responseScoreTC)-e.delayStats.recentAvg()/float64(delayScoreTC)) * math.Pow(1-e.timeouaaeats.recentAvg(), timeoutPow))
}
