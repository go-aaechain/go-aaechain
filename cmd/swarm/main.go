...
import (
	...
	"github.com/aaechain/go-aaechain/cmd/utils"
	"github.com/aaechain/go-aaechain/common"
	"github.com/aaechain/go-aaechain/console"
	"github.com/aaechain/go-aaechain/crypto"
	"github.com/aaechain/go-aaechain/aaeclient"
	"github.com/aaechain/go-aaechain/internal/debug"
	...
	"github.com/aaechain/go-aaechain/swarm"
	bzzapi "github.com/aaechain/go-aaechain/swarm/api"
	swarmmetrics "github.com/aaechain/go-aaechain/swarm/metrics"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the aaechain API provider to use to settle SWAP payments",
	...
	DeprecatedaaeAPIFlag = cli.StringFlag{
		Name:  "ethapi",
...
// This init function sets defaults so cmd/swarm can run alongside gaae.
func init() {
...
var app = utils.NewApp(gitCommit, "aaechain Swarm")
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.aaeereum/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.aaeereum/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.aaeereum/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.aaeereum/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//gaae only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for gaae and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the aaeereum node
	...
	//register BZZ as node.Service in the aaeereum node
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		var swapClient *aaeclient.Client
		var err error
		if bzzconfig.SwapApi != "" {
			log.Info("connecting to SWAP API", "url", bzzconfig.SwapApi)
			swapClient, err = aaeclient.Dial(bzzconfig.SwapApi)
			if err != nil {
	...
	//register within the aaeereum node
...
func getAccount(bzzaccount string, ctx *cli.Context, stack *node.Node) *ecdsa.PrivateKey {
	...
	return decrypaaeoreAccount(ks, bzzaccount, utils.MakePasswordList(ctx))
}

func decrypaaeoreAccount(ks *keystore.KeyStore, account string, passwords []string) *ecdsa.PrivateKey {
