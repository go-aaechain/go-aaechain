...
// makeWizard creates and returns a new puppaae wizard.
...
// setting up a new or managing an existing aaechain private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppaae, your aaechain private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new aaechain network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and aaestats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppaae uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering aaechain network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppaae", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somaaeing I can do")
		}
	}
}
