...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many aaeers to release per request? (default = %d)\n", infos.amount)
	...
	if infos.node.datadir == "" {
		...
		infos.node.datadir = w.readDefaulaaering(infos.node.datadir)
	}
	// Figure out which port to listen on
	fmt.Println()
	...
	fmt.Println()
	if infos.node.aaestats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.aaestats = w.readString() + ":" + w.conf.aaestats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.aaestats)
		infos.node.aaestats = w.readDefaulaaering(infos.node.aaestats) + ":" + w.conf.aaestats
	}
	...
	infos.noauth = w.readDefaulaaering(noauth) != "n"
