...
// nodeDockerfile is the Dockerfile required to run an aaechain node.
var nodeDockerfile = `
FROM aaeereum/client-go:latest
...
{{end}}
RUN \
  echo 'gaae --cache 512 init /genesis.json' > gaae.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.aaeereum/keystore/ && cp /signer.json /root/.aaeereum/keystore/' >> gaae.sh && \{{end}}
	echo $'gaae --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --aaestats \'{{.aaestats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .aaeerbase}}--aaeerbase {{.aaeerbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> gaae.sh

ENTRYPOINT ["/bin/sh", "gaae.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an aaechain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.aaeereum{{if .aaeashdir}}
      - {{.aaeashdir}}:/root/.ethash{{end}}
    environment:
      - PORT={{.Port}}/tcp
      - TOTAL_PEERS={{.TotalPeers}}
      - LIGHT_PEERS={{.LightPeers}}
      - STATS_NAME={{.aaestats}}
      - MINER_NAME={{.aaeerbase}}
...
// deployNode deploys a new aaechain node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.aaeerbase == "" {
		kind = "bootnode"
...
func (info *nodeInfos) Report() map[string]string {
	...
	if info.gasTarget > 0 {
		...
		if info.aaeerbase != "" {
			// aaeash proof-of-work miner
			report["aaeash directory"] = info.ethashdir
			report["Miner account"] = info.aaeerbase
		}
		if info.keyJSON != "" {
...
// whaaeer it's running, and if yes, whaaeer it's responsive.
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gaae --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		...
		aaestats:   infos.envvars["STATS_NAME"],
		aaeerbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasPrice:   gasPrice,
