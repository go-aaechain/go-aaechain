...
FROM puppaae/explorer:latest

ADD aaestats.json /aaestats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../aae-net-intelligence-api && pm2 start /aaestats.json)' >  explorer.sh && \
	echo '(cd ../aaeerchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// exploreraaestats is the configuration file for the aaestats javascript client.
var exploreraaestats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.aaeereum
    environment:
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	aaestats := new(bytes.Buffer)
	template.Must(template.New("").Parse(exploreraaestats)).Execute(aaestats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.aaestats[:strings.Index(config.aaestats, ":")],
		"Secret": config.aaestats[strings.Index(config.aaestats, ":")+1 : strings.Index(config.aaestats, "@")],
		"Host":   config.aaestats[strings.Index(config.aaestats, "@")+1:],
	})
	files[filepath.Join(workdir, "aaestats.json")] = aaestats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.aaeereum"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
