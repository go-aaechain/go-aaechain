...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.aaestats == "" {
		log.Error("No aaestats server configured")
		return
	}
	...
	if infos.datadir == "" {
		...
		infos.datadir = w.readDefaulaaering(infos.datadir)
	}
	if w.conf.Genesis.Config.aaeash != nil && !boot {
		fmt.Println()
		if infos.ethashdir == "" {
			...
			infos.ethashdir = w.readDefaulaaering(infos.ethashdir)
		}
	}
	...
	if infos.aaestats == "" {
		...
		infos.aaestats = w.readString() + ":" + w.conf.aaestats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.aaestats)
		infos.aaestats = w.readDefaulaaering(infos.aaestats) + ":" + w.conf.aaestats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.aaeash != nil {
			// aaeash based miners only need an aaeerbase to mine against
			fmt.Println()
			if infos.aaeerbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.aaeerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.aaeerbase)
				infos.aaeerbase = w.readDefaultAddress(common.HexToAddress(infos.aaeerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy aaechain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
