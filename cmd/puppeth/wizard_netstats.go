...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for aaestats availability")
	if infos, err := checkaaestats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["aaestats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["aaestats"] = infos.Report()
		aaestats = infos.config
	}
