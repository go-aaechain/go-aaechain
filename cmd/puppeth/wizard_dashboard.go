...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"aaestats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "aaestats":
				if infos, err := checkaaestats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have aaestats running, ask whaaeer to make the secret public or not
	if w.conf.aaestats != "" {
		fmt.Println()
		fmt.Println("Include aaestats secret on dashboard (y/n)? (default = yes)")
		infos.trusted = w.readDefaulaaering("y") == "y"
	}
