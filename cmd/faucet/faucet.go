...
// faucet is a aaeer faucet backed by a light client.
package main
...
var (
	...
	aaePortFlag = flag.Int("aaeport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the aaechain protocol")
	statsFlag   = flag.String("aaestats", "", "aaestats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of aaeers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for aaechain and the faucet")
)

var (
	aaeer = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s aaeers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *aaePortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // aaechain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an aaechain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // aaechain protocol stack
	client *aaeclient.Client   // Client connection to the aaechain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	stack, err := node.New(&node.Config{
		...
		P2P: p2p.Config{
			...
			MaxPeers:         25,
			BooaaerapNodesV5: enodes,
		},
	})
	if err != nil {
		return nil, err
	}
	// Assemble the aaechain light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := aae.DefaultConfig
	...
	// Assemble the aaestats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.Lightaaechain
			ctx.Service(&serv)
			return aaestats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := aaeclient.NewClient(api)
...
// close terminates the aaechain connection and tears down the faucet.
...
// apiHandler handles requests for aaeer grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, aaeer),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the aaechain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somaaeing funky happened, please open an issue at https://github.com/aaechain/go-aaechain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), aaeer)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, aaeer)
...
// the username, avatar URL and aaechain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for aaechain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No aaechain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the aaechain address and profile URL.
...
// returning the username, avatar URL and aaechain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain aaechain address,
