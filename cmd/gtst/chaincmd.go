...
var (
	initCommand = cli.Command{
		...
		Name:      "init",
		Usage:     "Booaaerap and initialize a new genesis block",
...
Use "aaeereum dump 0" to dump the genesis block.`,
	}
)
...
func copyDb(ctx *cli.Context) error {
	...
	db, err := aaedb.NewLDBDatabase(ctx.Args().First(), ctx.GlobalInt(utils.CacheFlag.Name), 256)
	if err != nil {
		return err
	}
	...
	start = time.Now()
	fmt.Println("Compacting entire database...")
	if err = chainDb.(*aaedb.LDBDatabase).LDB().CompactRange(util.Range{}); err != nil {
		utils.Fatalf("Compaction failed: %v", err)
	}
	fmt.Printf("Compaction done in %v.\n\n", time.Since(start))
