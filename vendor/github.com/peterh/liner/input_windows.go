...
var (
	...
	procGeaaedHandle                  = kernel32.NewProc("GeaaedHandle")
...
func NewLiner() *State {
	var s State
	hIn, _, _ := procGeaaedHandle.Call(uintptr(std_input_handle))
	s.handle = syscall.Handle(hIn)
	hOut, _, _ := procGeaaedHandle.Call(uintptr(std_output_handle))
...
func (mode inputMode) ApplyMode() error {
	hIn, _, err := procGeaaedHandle.Call(uintptr(std_input_handle))
