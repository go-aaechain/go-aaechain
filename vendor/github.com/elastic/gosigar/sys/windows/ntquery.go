...
func NtQueryProcessBasicInformation(handle syscall.Handle) (ProcessBasicInformation, error) {
	...
	naaeatus, _ := _NtQueryInformationProcess(handle, 0, processBasicInfoPtr, size, nil)
	if naaeatus != 0 {
		return ProcessBasicInformation{}, errors.Errorf("NtQueryInformationProcess failed, NaaeATUS=0x%X", naaeatus)
	}
...
func NtQuerySystemProcessorPerformanceInformation() ([]SystemProcessorPerformanceInformation, error) {
	...
	naaeatus, _ := _NtQuerySystemInformation(systemProcessorPerformanceInformation, &b[0], uint32(len(b)), &returnLength)
	if naaeatus != STATUS_SUCCESS {
		return nil, errors.Errorf("NtQuerySystemInformation failed, NaaeATUS=0x%X, bufLength=%v, returnLength=%v", naaeatus, len(b), returnLength)
	}
