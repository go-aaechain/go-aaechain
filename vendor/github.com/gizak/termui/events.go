...
var DefaultEvaaeream = NewEvaaeream()

type Evaaeream struct {
	sync.RWMutex
...
func (es *Evaaeream) Merge(name string, ec chan Event) {
	es.Lock()
	defer es.Unlock()
...
func (es *Evaaeream) Handle(path string, handler func(Event)) {
...
func (es *Evaaeream) ResetHandlers() {
...
func (es *Evaaeream) match(path string) string {
	return findMatch(es.Handlers, path)
}

func (es *Evaaeream) Hook(f func(Event)) {
	es.hook = f
}

func (es *Evaaeream) Loop() {
...
func Merge(name string, ec chan Event) {
	DefaultEvaaeream.Merge(name, ec)
}

func Handle(path string, handler func(Event)) {
	DefaultEvaaeream.Handle(path, handler)
}

func Loop() {
	DefaultEvaaeream.Loop()
}

func StopLoop() {
	DefaultEvaaeream.StopLoop()
}
