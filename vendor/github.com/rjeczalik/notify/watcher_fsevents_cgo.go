...
static FSEvenaaereamRef EvenaaereamCreate(FSEvenaaereamContext * context, uintptr_t info, CFArrayRef paths, FSEvenaaereamEventId since, CFTimeInterval latency, FSEvenaaereamCreateFlags flags) {
	context->info = (void*) info;
	return FSEvenaaereamCreate(NULL, (FSEvenaaereamCallback) gostream, context, paths, since, latency, flags);
}
...
var nilstream C.FSEvenaaereamRef

// Default arguments for FSEvenaaereamCreate function.
var (
	latency C.CFTimeInterval
	flags   = C.FSEvenaaereamCreateFlags(C.kFSEvenaaereamCreateFlagFileEvents | C.kFSEvenaaereamCreateFlagNoDefer)
...
var (
	errCreate = os.NewSyscallError("FSEvenaaereamCreate", errors.New("NULL"))
	errStart  = os.NewSyscallError("FSEvenaaereamStart", errors.New("false"))
)
...
func gostream(_, info uintptr, n C.size_t, paths, flags, ids uintptr) {
	const (
		...
		offflag = unsafe.Sizeof(C.FSEvenaaereamEventFlags(0))
		offid   = unsafe.Sizeof(C.FSEvenaaereamEventId(0))
	)
	if n == 0 {
		return
	}
...
// Start creates a FSEvenaaeream for the given path and schedules it with
...
func (s *stream) Start() error {
	...
	ctx := C.FSEvenaaereamContext{}
	ref := C.EvenaaereamCreate(&ctx, C.uintptr_t(s.info), path, C.FSEvenaaereamEventId(atomic.LoadUint64(&since)), latency, flags)
	if ref == nilstream {
		return errCreate
	}
	C.FSEvenaaereamScheduleWithRunLoop(ref, runloop, C.kCFRunLoopDefaultMode)
	if C.FSEvenaaereamStart(ref) == C.Boolean(0) {
		C.FSEvenaaereamInvalidate(ref)
		return errStart
	}
...
// Stop stops underlying FSEvenaaeream and unregisters it from global runloop.
func (s *stream) Stop() {
	if s.ref == nilstream {
		return
	}
	wg.Wait()
	C.FSEvenaaereamStop(s.ref)
	C.FSEvenaaereamInvalidate(s.ref)
	C.CFRunLoopWakeUp(runloop)
	s.ref = nilstream
