...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gaae.LightScryptN, Gaae.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.aaeereum", new NodeConfig());
			node.start();
			...
			aaechainClient ec = node.getaaechainClient();
	...
	// signatures too. Regression test for go-aaeereum issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogaaeer
	...
	workspace, err := ioutil.TempDir("", "gaae-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/aaeereum/gaaetest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gaae and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.aaeereum", "github.com/aaechain/go-aaechain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gaae.aar"), "gaae.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "aaeereum", "gaaetest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
