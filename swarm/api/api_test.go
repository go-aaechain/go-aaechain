...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.aae"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.aae"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.aae"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	aaeAddr := "swarm.aae"
	aaeHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	aaeResolve := newTestResolver(aaeHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(aaeResolve, "")),
			addr:   aaeAddr,
			result: aaeHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(aaeResolve, ""),
				MultiResolverOptionWithResolver(aaeResolve, ""),
			),
			addr:   aaeAddr,
			result: aaeHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(aaeResolve, "aae"),
			),
			addr:   aaeAddr,
			result: aaeHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "aae"),
				MultiResolverOptionWithResolver(doesntResolve, "aae"),
				MultiResolverOptionWithResolver(aaeResolve, "aae"),
			),
			addr:   aaeAddr,
		...
		},
		{
			...
			addr:   aaeAddr,
			result: aaeHash,
			err:    errors.New(`DNS name not found: "swarm.aae"`),
		},
		{
