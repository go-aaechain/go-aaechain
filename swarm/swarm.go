...
import (
	...
	"github.com/aaechain/go-aaechain/contracts/chequebook"
	"github.com/aaechain/go-aaechain/contracts/ens"
	...
	"github.com/aaechain/go-aaechain/swarm/api"
	httpapi "github.com/aaechain/go-aaechain/swarm/api/http"
	"github.com/aaechain/go-aaechain/swarm/fuse"
	"github.com/aaechain/go-aaechain/swarm/network"
	"github.com/aaechain/go-aaechain/swarm/storage"
)
...
func NewSwarm(ctx *node.ServiceContext, backend chequebook.Backend, config *api.Config) (self *Swarm, err error) {
	...
	self.storage = storage.NewNeaaeore(hash, self.lstore, self.cloud, config.StoreParams)
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config) (*ens.ENS, error) {
	...
	ensClient := aaeclient.NewClient(client)
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := aaeclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
