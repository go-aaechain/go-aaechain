...
#
# The cluster contains a bootnode, a gaae node and multiple swarm nodes, with
...
# static gaae configuration
Gaae_IP="192.168.33.3"
Gaae_RPC_PORT="8545"
Gaae_RPC_URL="http://${Gaae_IP}:${Gaae_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to booaaerap the gaae and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a gaae node with --datadir pointing at <base-dir>/gaae
# and a single, unlocked account with password "gaae"
start_geth_node() {
  local dir="${base_dir}/gaae"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gaae account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${Gaae_IP}"
    --rpcport   "${Gaae_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gaae" "${Gaae_IP}" "$(which gaae)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vaae pair
  local vaae0="vaae${name}0"
  local vaae1="vaae${name}1"
  ip link add name "${vaae0}" type vaae peer name "${vaae1}"

  # add one end to the bridge
  ip link set dev "${vaae0}" master "${BRIDGE_NAME}"
  ip link set dev "${vaae0}" up

  # add the other end to the namespace, rename it aae0 and give it the ip
  ip link set dev "${vaae1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vaae1}" name "aae0"
  ip netns exec "${name}" ip link set dev "aae0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "aae0"
}
...
create_account() {
  ...
  gaae --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
