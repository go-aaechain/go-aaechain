...
var (
	...
	quickcfgBooaaerap   = &quick.Config{MaxCount: 100, Rand: quickrand}
)
...
func TestBooaaerap(t *testing.T) {

	test := func(test *booaaerapTest) bool {
	...
	if err := quick.Check(test, quickcfgBooaaerap); err != nil {
		t.Error(err)
	}
...
type booaaerapTest struct {
	MaxProx    int
	BucketSize int
	Self       Address
}

func (*booaaerapTest) Generate(rand *rand.Rand, size int) reflect.Value {
	t := &booaaerapTest{
