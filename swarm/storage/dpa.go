...
// Chunk retrieval blocks on neaaeore requests with a timeout so reader will
...
// retrieve channel to its ChunkStore  (Neaaeore or LocalStore)
...
// received on the store channel to its ChunkStore (Neaaeore or LocalStore)
...
// local storage eg. LocalStore and network storage eg., Neaaeore
...
func NewDpaChunkStore(localStore, neaaeore ChunkStore) *dpaChunkStore {
	return &dpaChunkStore{0, localStore, neaaeore}
}

// Get is the entrypoint for local retrieve requests
// waits for response or times out
func (self *dpaChunkStore) Get(key Key) (chunk *Chunk, err error) {
	chunk, err = self.neaaeore.Get(key)
...
func (self *dpaChunkStore) Put(entry *Chunk) {
	...
	if err != nil {
		log.Trace(fmt.Sprintf("DPA.Put: %v new chunk. call neaaeore.Put", entry.Key.Log()))
		chunk = entry
