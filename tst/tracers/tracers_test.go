...
import (
	...
	"github.com/aaechain/go-aaechain/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gaae console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = aae.getBlock(aae.getTransaction(tx).blockHash);
  var genesis = aae.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.aae.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  aae.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
