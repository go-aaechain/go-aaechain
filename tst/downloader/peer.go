...
type peerConnection struct {
	...
	receipaaearted time.Time // Time instance when the last receipt fetch was started
	...
	version int        // aae protocol version number to switch strategies
...
func (p *peerConnection) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [aae/62+] requested on aae/%d", p.version))
	}
...
func (p *peerConnection) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [aae/62+] requested on aae/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [aae/63+] requested on aae/%d", p.version))
	}
	// Short circuit if the peer is already fetching
	if !atomic.CompareAndSwapInt32(&p.receiptIdle, 0, 1) {
		return errAlreadyFetching
	}
	p.receipaaearted = time.Now()
...
func (p *peerConnection) FetchNodeData(hashes []common.Hash) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [aae/63+] requested on aae/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) SetReceiptsIdle(delivered int) {
	p.setIdle(p.receipaaearted, delivered, &p.receiptThroughput, &p.receiptIdle)
}
...
// Lacks retrieves whaaeer the hash of a blockchain item is on the peers lacking
// list (i.e. whaaeer we know that the peer does not have it).
