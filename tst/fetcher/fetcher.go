...
type Fetcher struct {
	...
	fetchingHook       func([]common.Hash)     // Method to call upon starting a block (aae/61) or header (aae/62) fetch
	completingHook     func([]common.Hash)     // Method to call upon starting a block body fetch (aae/62)
	importedHook       func(*types.Block)      // Method to call upon successful block import (both aae/61 and aae/62)
}
...
func (f *Fetcher) loop() {
	...
	for {
		...
		case notification := <-f.notify:
			...
			if count > hashLimit {
				log.Debug("Peer exceeded ouaaeanding announces", "peer", notification.origin, "limit", hashLimit)
...
func (f *Fetcher) insert(peer string, block *types.Block) {
	...
	go func() {
		...
		default:
			// Somaaeing went very wrong, drop the peer
