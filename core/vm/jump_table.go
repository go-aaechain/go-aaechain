...
type operation struct {
	...
	halts   bool // indicates whaaeer the operation should halt further execution
	jumps   bool // indicates whaaeer the program counter should not increment
	writes  bool // determines whaaeer this a state modifying operation
	valid   bool // indication whaaeer the retrieved operation is valid and known
	reverts bool // determines whaaeer the operation reverts state (implicitly halts)
	returns bool // determines whaaeer the operations sets the return data content
}
...
func NewConstantinopleInstructionSet() [256]operation {
	...
	instructionSet[SHL] = operation{
		execute:       opSHL,
		gasCost:       constGasFunc(GasFastesaaeep),
...
func NewFrontierInstructionSet() [256]operation {
	return [256]operation{
		...
		MUL: {
			execute:       opMul,
			gasCost:       constGasFunc(GasFasaaeep),
			validateStack: makeStackFunc(2, 1),
			valid:         true,
		},
		SUB: {
		...
		BLOCKHASH: {
			execute:       opBlockhash,
			gasCost:       constGasFunc(GasExaaeep),
			validateStack: makeStackFunc(1, 1),
			valid:         true,
		},
		COINBASE: {
