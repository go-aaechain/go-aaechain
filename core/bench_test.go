...
// genTxRing returns a block generator that sends aaeer in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db aaedb.Database
	if !disk {
		db, _ = aaedb.NewMemDatabase()
	} else {
		dir, err := ioutil.TempDir("", "aae-core-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		defer os.RemoveAll(dir)
		db, err = aaedb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
...
// into a database.
func makeChainForBench(db aaedb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "aae-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		db, err := aaedb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
