...
var (
	...
	configPrefix   = []byte("aaeereum-config-") // config prefix for the db
...
func WriteCanonicalHash(db aaedb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db aaedb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db aaedb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db aaedb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db aaedb.Putter, count uint64) error {
...
func WriteHeader(db aaedb.Putter, header *types.Header) error {
...
func WriteBody(db aaedb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db aaedb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db aaedb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db aaedb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db aaedb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db aaedb.Putter, block *types.Block) error {
...
func WriteBloomBits(db aaedb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db aaedb.Database) aaedb.Database {
	return aaedb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db aaedb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db aaedb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db aaedb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
