...
// to the aae network. The implementations of the functions should be non-blocking.
//
...
// currenaaeate returns the light state of the current head header
func (pool *TxPool) currenaaeate(ctx context.Context) *state.StateDB {
...
// seaaeate sets the status of a tx to either recently mined or recently rolled back
func (txc txStateChanges) seaaeate(txHash common.Hash, mined bool) {
	val, ent := txc[txHash]
	if ent && (val != mined) {
		delete(txc, txHash)
	} else {
...
// validateTx checks whaaeer a transaction is valid according to the consensus rules.
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	currenaaeate := pool.currenaaeate(ctx)
	if n := currenaaeate.GetNonce(from); n > tx.Nonce() {
	...
	// cost == V + GP * GL
	if b := currenaaeate.GetBalance(from); b.Cmp(tx.Cost()) < 0 {
