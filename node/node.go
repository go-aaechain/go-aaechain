...
func (n *Node) OpenDatabase(name string, cache, handles int) (aaedb.Database, error) {
	if n.config.DataDir == "" {
		return aaedb.NewMemDatabase()
	}
	return aaedb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
