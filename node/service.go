...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (aaedb.Database, error) {
	if ctx.config.DataDir == "" {
		return aaedb.NewMemDatabase()
	}
	db, err := aaedb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
