...
creating a collection of devp2p nodes, connecting them togaaeer to form a
...
connecting two nodes togaaeer).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of somaaeing happening in the network as a result of a
...
* `Expect` - an expectation function which returns whaaeer or not a
...
As a concrete example, consider a simulated network of aaechain nodes. An
