...
func (t *TestAPI) Geaaeate() []byte {
	return t.state.Load().([]byte)
}

func (t *TestAPI) Seaaeate(state []byte) {
	t.state.Store(state)
}
...
func TestHTTPSnapshot(t *testing.T) {
	...
	for i, node := range nodes {
		...
		if err := rpc.Call(nil, "test_seaaeate", []byte(state)); err != nil {
	...
	for i, state := range states {
		goaaeate := snap.Nodes[i].Snapshots["test"]
		if string(goaaeate) != state {
			t.Fatalf("expected snapshot state %q, got %q", state, goaaeate)
		}
	}
	...
	for i, node := range nodes {
		...
		defer rpc.Close()
		var state []byte
		if err := rpc.Call(&state, "test_geaaeate"); err != nil {
