...
func newTickeaaeore() *tickeaaeore {
	return &tickeaaeore{
...
func (s *tickeaaeore) addTopic(topic Topic, register bool) {
...
func (s *tickeaaeore) addSearchTopic(t Topic, foundChn chan<- *Node) {
	s.addTopic(t, false)
...
func (s *tickeaaeore) removeSearchTopic(t Topic) {
...
func (s *tickeaaeore) removeRegisterTopic(topic Topic) {
...
func (s *tickeaaeore) regTopicSet() []Topic {
...
func (s *tickeaaeore) nextRegisterLookup() (lookupInfo, time.Duration) {
...
func (s *tickeaaeore) nextSearchLookup(topic Topic) lookupInfo {
	tr := s.radius[topic]
...
func (s *tickeaaeore) ticketsInWindow(topic Topic) []ticketRef {
...
func (s *tickeaaeore) removeExcessTickets(t Topic) {
...
// Less reports whaaeer the element with
...
func (s *tickeaaeore) addTicketRef(r ticketRef) {
...
func (s *tickeaaeore) nextFilteredTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
	for {
...
func (s *tickeaaeore) ticketRegistered(ref ticketRef) {
	now := mclock.Now()
...
func (s *tickeaaeore) nextRegisterableTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
...
func (s *tickeaaeore) removeTicketRef(ref ticketRef) {
...
func (s *tickeaaeore) registerLookupDone(lookup lookupInfo, nodes []*Node, ping func(n *Node) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
...
func (s *tickeaaeore) searchLookupDone(lookup lookupInfo, nodes []*Node, query func(n *Node, topic Topic) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
		if i == 0 || (binary.BigEndian.Uint64(n.sha[:8])^binary.BigEndian.Uint64(lookup.target[:8])) < s.radius[lookup.topic].minRadius {
			if lookup.radiusLookup {
...
func (s *tickeaaeore) adjustWithTicket(now mclock.AbsTime, targetHash common.Hash, t *ticket) {
...
func (s *tickeaaeore) addTicket(localTime mclock.AbsTime, pingHash []byte, ticket *ticket) {
...
func (s *tickeaaeore) getNodeTicket(node *Node) *ticket {
	if s.nodes[node] == nil {
...
func (s *tickeaaeore) canQueryTopic(node *Node, topic Topic) bool {
...
func (s *tickeaaeore) addTopicQuery(hash common.Hash, node *Node, lookup lookupInfo) {
	now := mclock.Now()
	qq := s.queriesSent[node]
	if qq == nil {
...
func (s *tickeaaeore) cleanupTopicQueries(now mclock.AbsTime) {
...
func (s *tickeaaeore) gotTopicNodes(from *Node, hash common.Hash, nodes []rpcNode) (timeout bool) {
	now := mclock.Now()
